...
def test_gemini_provider_requires_api_key() -> None:
    with pytest.raises(ConfigurationError, match="Gemini API key is required"):
        load_application_config(
            environ={"AI_RA_API_PROVIDER": "gemini"},
            load_dotenv=False,
            set_as_current=False,
        )


def test_gemini_provider_uses_cloud_endpoint_by_default() -> None:
    config = load_application_config(
        environ={
            "AI_RA_API_PROVIDER": "gemini",
            "AI_RA_API_KEY": "test-key",
            "AI_RA_MODEL": "gemini-test",
        },
        load_dotenv=False,
        set_as_current=False,
    )
    assert config.llm.provider == "gemini"
    assert config.llm.base_url.startswith("https://generativelanguage.googleapis.com/")
