...
SUPPORTED_RUNTIME_PROVIDERS: Final[frozenset[str]] = frozenset({"gemini", "ollama"})
...
DEFAULT_GEMINI_BASE_URL: Final[str] = "https://generativelanguage.googleapis.com/v1beta"
...
class LLMRuntimeConfig:
    ...
    Live analysis supports Ollama and Google Gemini.
    """
...
def validate_application_config(config: ApplicationConfig) -> ApplicationConfig:
    ...
    if provider == "gemini" and not config.llm.api_key.strip():
        raise ConfigurationError(
            "Gemini API key is required when provider is 'gemini'.",
            field="llm.api_key",
        )
