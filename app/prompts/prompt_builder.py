...
class PromptBuilder:
    ...
    def build(
        ...
        # Some short papers, proposals, preprints, and poorly tagged PDFs do
        # not contain standalone headings that match the section extractor.
        # Never reduce such a document to its title alone: use bounded raw
        # text so the resulting critique and proposal insights remain valid.
        extracted_body_characters = sum(
            len(value)
            for value in available_sections.values()
        )
        if extracted_body_characters < 500:
            context = (
                f"# Title\n\n{paper.title}\n\n"
                f"# Full Paper Text\n\n{paper.text[:12000]}"
            )
            _LOGGER.warning(
                "Structured section extraction produced only %d body "
                "characters; using bounded full-text context.",
                extracted_body_characters,
            )
